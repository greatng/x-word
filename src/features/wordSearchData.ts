...
export async function fetchWordSearchEntries(
    ...
    const response = await fetch(
        'https://jlpt-vocab-api.vercel.app/api/graphql',
        {
            method: 'POST',
            headers: {
    ...
    if (!response.ok) {
        throw new Error('Failed to fetch JLPT vocabulary');
    }

    const payload = await response.json();
    const words = payload?.data?.words?.words ?? [];

    return words
        .map((item: { word?: string; meaning?: string; furigana?: string }) => {
            const boardWord = item.furigana?.trim() || item.word?.trim() || '';
